...
export const metadata: Metadata = {
  // Admin layout already applies: "%s | Admin | Kasi Flavors"
  title: "Platform overview",
  description:
    "View platform-wide orders, revenue, store performance, and status breakdowns across all Kasi Flavors stores.",
  alternates: {
    canonical: "/admin",
  },
  openGraph: {
    ...
    description:
      "Monitor platform-wide performance including revenue, order volume, prep times, and store activity across Kasi Flavors.",
    url: "/admin",
  },
  twitter: {
    card: "summary",
    title: "Platform overview | Admin | Kasi Flavors",
    description:
      "Admin overview of orders, revenue, and store performance across the Kasi Flavors platform.",
  },
  // robots are already set to noindex,nofollow on the admin layout
};
...
export default async function AdminOverviewPage({
  ...
  const allStoreRows: StoreAgg[] = stores.map((s) => {
    ...
    if (agg) {
      return {
        ...agg,
        avgOrderCents:
          agg.orders > 0 ? Math.round(agg.revenueCents / agg.orders) : 0,
      };
    }
    return {
      store: s,
  ...
  const topStores = [...allStoreRows].sort(
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex w-full items-center justify-between gap-2 sm:w-auto">
            ...
            <div className="-mx-4 flex flex-1 items-center gap-2 overflow-x-auto px-4 sm:mx-0 sm:flex-none sm:overflow-visible">
              <RangeChip href="/admin?range=7d" active={rangeParam === "7d"}>
                7<span className="max-sm:hidden"> days</span>
              </RangeChip>
              <RangeChip href="/admin?range=30d" active={rangeParam === "30d"}>
                30<span className="max-sm:hidden"> days</span>
              </RangeChip>
              <RangeChip href="/admin?range=all" active={rangeParam === "all"}>
                All<span className="max-sm:hidden"> time</span>
              </RangeChip>
            </div>
        ...
        <section>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                Overview of all stores on the platform.
              </p>
            </div>
          </div>
          ...
          {topStores.length === 0 ? (
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {topStores.map((row) => (
                <article
                  key={row.store.id}
                  className="overflow-hidden rounded-xl border border-slate-100 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900"
                >
                  ...
                  <div className="mt-3 flex items-center justify-between gap-2">
                    <Link
                      href={`/admin/stores/${row.store.id}`}
                      className="inline-flex items-center rounded-md px-3 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:text-slate-300"
                    >
                      Manage
                    </Link>
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400">
                      {row.lastOrderAt
                        ? new Date(row.lastOrderAt).toLocaleString("en-ZA", {
                            day: "2-digit",
                            month: "short",
                            hour: "2-digit",
                            minute: "2-digit",
